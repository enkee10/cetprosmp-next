...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grupo_modulos_grupo_id_modulo_id_instancia_key'
      AND conrelid = 'public.grupo_modulos'::regclass
  ) THEN
    ALTER TABLE public.grupo_modulos
      ADD CONSTRAINT grupo_modulos_grupo_id_modulo_id_instancia_key
      UNIQUE (grupo_id, modulo_id, instancia);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'modulos_estudiantes_matricula_id_grupo_modulo_id_key'
      AND conrelid = 'public.modulos_estudiantes'::regclass
  ) THEN
    ALTER TABLE public.modulos_estudiantes
      ADD CONSTRAINT modulos_estudiantes_matricula_id_grupo_modulo_id_key
      UNIQUE (matricula_id, grupo_modulo_id);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS modulos_estudiantes_grupo_modulo_id_idx
