...
## GetUserByDocumentId
You can execute the `GetUserByDocumentId` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetUserByDocumentId(dc: DataConnect, vars: GetUserByDocumentIdVariables, options?: useDataConnectQueryOptions<GetUserByDocumentIdData>): UseDataConnectQueryResult<GetUserByDocumentIdData, GetUserByDocumentIdVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetUserByDocumentId(vars: GetUserByDocumentIdVariables, options?: useDataConnectQueryOptions<GetUserByDocumentIdData>): UseDataConnectQueryResult<GetUserByDocumentIdData, GetUserByDocumentIdVariables>;
```
...
```
### Return Type
Recall that calling the `GetUserByDocumentId` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetUserByDocumentId` Query is of type `GetUserByDocumentIdData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetUserByDocumentId`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetUserByDocumentIdVariables } from '@dataconnect/generated';
import { useGetUserByDocumentId } from '@dataconnect/generated/react'

export default function GetUserByDocumentIdComponent() {
  // The `useGetUserByDocumentId` Query hook requires an argument of type `GetUserByDocumentIdVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetUserByDocumentId(getUserByDocumentIdVars);
  // Variables can be defined inline as well.
  const query = useGetUserByDocumentId({ documentId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetUserByDocumentId(dataConnect, getUserByDocumentIdVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetUserByDocumentId(getUserByDocumentIdVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetUserByDocumentId(dataConnect, getUserByDocumentIdVars, options);
...
## ListActEconomicas
You can execute the `ListActEconomicas` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListActEconomicas(dc: DataConnect, options?: useDataConnectQueryOptions<ListActEconomicasData>): UseDataConnectQueryResult<ListActEconomicasData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListActEconomicas(options?: useDataConnectQueryOptions<ListActEconomicasData>): UseDataConnectQueryResult<ListActEconomicasData, undefined>;
```

### Variables
The `ListActEconomicas` Query has no variables.
### Return Type
Recall that calling the `ListActEconomicas` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListActEconomicas` Query is of type `ListActEconomicasData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListActEconomicas`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useListActEconomicas } from '@dataconnect/generated/react'

export default function ListActEconomicasComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListActEconomicas();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListActEconomicas(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListActEconomicas(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListActEconomicas(dataConnect, options);
  ...
  // If the Query is successful, you can access the data returned using the `UseQueryResult.data` field.
  if (query.isSuccess) {
    console.log(query.data.actEconomicas);
  }
  return <div>Query execution {query.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
...
## GetPostById
You can execute the `GetPostById` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetPostById(dc: DataConnect, vars: GetPostByIdVariables, options?: useDataConnectQueryOptions<GetPostByIdData>): UseDataConnectQueryResult<GetPostByIdData, GetPostByIdVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetPostById(vars: GetPostByIdVariables, options?: useDataConnectQueryOptions<GetPostByIdData>): UseDataConnectQueryResult<GetPostByIdData, GetPostByIdVariables>;
```
...
```
### Return Type
Recall that calling the `GetPostById` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetPostById` Query is of type `GetPostByIdData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetPostById`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetPostByIdVariables } from '@dataconnect/generated';
import { useGetPostById } from '@dataconnect/generated/react'

export default function GetPostByIdComponent() {
  // The `useGetPostById` Query hook requires an argument of type `GetPostByIdVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetPostById(getPostByIdVars);
  // Variables can be defined inline as well.
  const query = useGetPostById({ id: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetPostById(dataConnect, getPostByIdVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetPostById(getPostByIdVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetPostById(dataConnect, getPostByIdVars, options);
...
export default function CreatePostComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ titulo: ..., slug: ..., tipo: ..., contenido: ..., resumen: ..., imagenPortadaUrl: ..., estado: ..., comentariosActivos: ..., entidadTipo: ..., entidadId: ..., fechaPublicacion: ..., });
...
export default function UpdatePostComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ id: ..., titulo: ..., slug: ..., tipo: ..., contenido: ..., resumen: ..., imagenPortadaUrl: ..., estado: ..., comentariosActivos: ..., entidadTipo: ..., entidadId: ..., fechaPublicacion: ..., });
